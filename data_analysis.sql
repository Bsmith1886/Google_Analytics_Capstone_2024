...
--Creating a view with ordered month and days

CREATE VIEW `lively-lock-439218-m3.Bicycle_data_2022_2023.week_ordered_view_fixed` as
SELECT *,
  CASE day_of_week
    WHEN 'sunday' THEN 1
    WHEN 'monday' THEN 2
...
SELECT member_casual, month, count(ride_id) as total_rides,
  FROM `lively-lock-439218-m3.Bicycle_data_2022_2023.week_ordered_view_fixed`
  group by month_order, month, member_casual
  order by month_order asc;

--Member type by Rides per week
SELECT member_casual, day_of_week, count(ride_id) as total_rides,
  FROM `lively-lock-439218-m3.Bicycle_data_2022_2023.week_ordered_view_fixed`
  group by day_order, day_of_week,member_casual
  order by day_order asc;

--Member type by rides per week, average duration and median duration

SELECT member_casual, day_of_week, count(ride_id) as total_rides, avg(duration_in_minutes) as average_duration, APPROX_QUANTILES(duration_in_minutes, 2)[OFFSET(1)] AS median_duration
  FROM `lively-lock-439218-m3.Bicycle_data_2022_2023.week_ordered_view_fixed`
  group by day_order, day_of_week,member_casual
  order by day_order asc;
